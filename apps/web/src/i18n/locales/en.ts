export default {
  app: {
    brand: 'Bagheera',
  },
  common: {
    confirmTitle: 'Confirmation',
    confirmBody: 'Do you confirm?',
    ok: 'Ok',
    cancel: 'Cancel',
    or: 'or',
    pager: {
      label: 'Pagination',
      previous: 'Previous',
      next: 'Next',
      status: 'Page {page} of {pageCount}',
    },
  },
  chartRange: {
    label: 'Range',
    months12: '12 months',
    months24: '24 months',
    all: 'All time',
  },
  auth: {
    signIn: {
      title: 'Sign in',
      submit: 'Sign in',
      passkeySubmit: 'Sign in with a passkey',
      passkeyHint: 'Your browser will offer the passkeys saved for this site — pick yours.',
      invalidCredentials: 'Sign-in failed. Please try again.',
      passkeysUnsupported: "This browser doesn't support passkeys, so sign-in isn't available.",
      registerLink: 'Create an account',
      success: 'Signed in.',
      tooManyAttempts: 'Too many attempts. Please wait a minute and try again.',
    },
    register: {
      title: 'Create an account',
      email: 'Email address',
      country: 'Country',
      submit: 'Continue',
      passkeyHint: "You'll register a passkey to finish creating your account.",
      success:
        "If this email isn't already registered, you'll receive a link to create your account.",
      genericError: 'Something went wrong. Please try again.',
      signInLink: 'Already have an account? Sign in',
    },
    activate: {
      title: 'Create your account',
      intro: 'Click below to create a passkey and finish setting up your account.',
      submit: 'Continue',
      success: 'Account created. You are now signed in.',
      error: 'This link is invalid or has expired.',
      signInLink: 'Go to sign in',
    },
    confirmEmailChange: {
      title: 'Confirm email change',
      pending: 'Confirming your new email address…',
      success: 'Your email address has been updated.',
      error: 'Email change error (link expired or already used?)',
    },
    validation: {
      required: 'This field is required.',
      email: 'Enter a valid email address.',
      country: 'Country must be a 2-letter code.',
    },
  },
  nav: {
    home: 'Dashboard',
    accounts: 'Accounts',
    reports: 'Reports',
    settings: 'Settings',
  },
  home: {
    signedInAs: 'Connected as {email}',
    accountMenu: 'Account menu',
    signOut: 'Logout',
    accountsLink: 'Accounts',
    profileLink: 'Profile settings',
    passkeysLink: 'Passkeys',
  },
  accounts: {
    title: 'Accounts',
    empty: "You don't have any bank yet.",
    addAccount: 'New account',
    editTitle: 'Edit account',
    editBankTitle: 'Edit bank',
    bankNameLabel: 'Bank name',
    noAccountsForBank: 'No account',
    bank: 'Bank',
    existingBank: 'Use an existing bank',
    chooseBank: 'Choose a bank…',
    newBankName: 'Or create a new bank',
    accountName: 'Account name',
    account: 'Account',
    balance: 'Balance',
    currency: 'Currency',
    chooseCurrency: 'Choose a currency…',
    initialBalance: 'Initial balance',
    next: 'Next',
    submit: 'Save',
    edit: 'Edit',
    close: 'Close',
    delete: 'Delete',
    closed: 'Closed',
    deleted: 'Deleted',
    bankSaved: 'Bank saved',
    bankClosed: 'Bank closed',
    bankDeleted: 'Bank deleted',
    accountSaved: 'Account saved',
    accountClosed: 'Account closed',
    accountDeleted: 'Account deleted',
    genericError: 'Something went wrong. Please try again.',
    validation: {
      bankChoiceRequired: 'You must select a bank.',
      currency: 'Enter a 3-letter currency code.',
    },
  },
  operations: {
    title: 'Operations',
    createTitle: 'New operation',
    editTitle: 'Edit operation',
    addOperation: 'New operation',
    debit: 'Debit',
    credit: 'Credit',
    thirdParty: 'Third party',
    amount: 'Amount',
    category: 'Category',
    noCategory: 'No category',
    paymentMethod: 'Payment method',
    choosePaymentMethod: 'Choose a payment method…',
    transferAccount: 'Transfer to account',
    chooseTransferAccount: 'Choose an account…',
    externalAccount: 'External account',
    balance: 'Balance',
    generatedByScheduler: 'Generated by a scheduler',
    firstOperationTip: 'Here you are! Just add your first operation.',
    valueDate: 'Value date',
    notes: 'Notes',
    reconciled: 'Reconciled',
    submit: 'Save',
    submitAndNew: 'Save & add another',
    saved: 'Operation saved',
    edit: 'Edit',
    genericError: 'Something went wrong. Please try again.',
    empty: 'No operations yet.',
    schedulersLink: 'Scheduled operations',
    validation: {
      amount: 'Enter an amount greater than zero.',
      amountTooHigh: 'Enter a smaller amount.',
    },
    batch: {
      delete: 'Delete selected',
      reconcile: 'Reconcile selected',
      deleted: 'Operations deleted',
      reconciled: 'Operations reconciled',
    },
    search: {
      title: 'Search operations',
      show: 'Search',
      activeHint: 'A search filter is active',
      any: 'Any',
      yes: 'Reconciled only',
      no: 'Not reconciled only',
      both: 'Reconciled & not reconciled',
      type: 'Type',
      amount: 'Amount',
      dateFrom: 'From date',
      dateTo: 'To date',
      submit: 'Search',
      clear: 'Clear search',
      operators: {
        gt: 'Greater than',
        gte: 'At least',
        lt: 'Less than',
        lte: 'At most',
        eq: 'Equal to',
      },
    },
  },
  schedulers: {
    title: 'Scheduled operations',
    createTitle: 'New scheduled operation',
    editTitle: 'Edit scheduled operation',
    addScheduler: 'New scheduler',
    firstOccurrence: 'First occurrence',
    every: 'Every',
    everyN: 'Every {count} {unit}',
    frequencyUnit: 'Frequency',
    units: {
      day: 'day(s)',
      week: 'week(s)',
      month: 'month(s)',
      year: 'year(s)',
    },
    limitDate: 'Stop after (optional)',
    active: 'Active',
    paused: 'Paused',
    created: 'Scheduler saved',
    updated: 'Scheduler saved',
    genericError: 'Something went wrong. Please try again.',
    empty: 'No scheduled operations yet.',
    validation: {
      frequencyValue: 'Enter a frequency of at least 1.',
    },
    batch: {
      delete: 'Delete selected',
      deleted: 'Schedulers deleted',
    },
  },
  reports: {
    title: 'Reports',
    createTitle: 'New report',
    editTitle: 'Edit report',
    newSumReport: 'New sum report',
    newAverageReport: 'New average report',
    newDistributionReport: 'New distribution report',
    type: 'Type',
    typeHint: {
      sum: 'Adds up credits and debits for each period.',
      average: 'Averages credits and debits for each period.',
      distribution:
        'Ranks operations by category, third party, or payment method — over the whole range, or broken down over time.',
    },
    reportTitle: 'Title',
    reconciledOnly: 'Reconciled only',
    reconciledOnlyHint: 'Only include operations you have reconciled.',
    thirdPartiesHint: 'Matches any third party containing this text.',
    sum: 'Sum',
    average: 'Average',
    distribution: 'Distribution',
    homepage: 'Show on the homepage',
    dateFrom: 'From date',
    dateTo: 'To date',
    accounts: 'Accounts',
    accountsHint: 'Leave empty to include every account.',
    categoriesHint: 'Leave empty to include every category.',
    dateRangeInvalid: 'End date must be on or after the start date.',
    significantResultsNumberInvalid: 'Enter a whole number between 1 and {max}.',
    periodGrouping: 'Break down over',
    periodGroupingHint: '"Whole period" groups every matching operation into a single value.',
    periodGroupingHintDistribution:
      '"Whole period" ranks once over the whole range. Month/quarter/year instead shows the same top results stacked over time.',
    periods: {
      month: 'Month',
      quarter: 'Quarter',
      year: 'Year',
      all: 'Whole period',
    },
    dataGrouping: 'Break down by',
    dataGroupingHint: 'Which dimension operations are ranked and grouped by.',
    dataGroupingOptions: {
      category: 'Category',
      thirdParty: 'Third party',
      paymentMethod: 'Payment method',
    },
    significantResultsNumber: 'Number of results shown',
    significantResultsNumberHint:
      'The rest is combined into a single "Other" bucket, along with any uncategorized operations.',
    other: 'Other',
    viewChart: 'View chart',
    hideChart: 'Hide chart',
    saved: 'Report saved',
    deleted: 'Report deleted',
    genericError: 'Something went wrong. Please try again.',
    empty: 'No reports yet.',
    batch: {
      delete: 'Delete selected',
      deleted: 'Reports deleted',
    },
  },
  dashboard: {
    title: 'Dashboard',
    subtitle: "Here's where things stand today.",
    reportsLink: 'Reports',
    onboardingNoBank: 'Welcome to Bagheera! Now, create your first bank.',
    onboardingNoAccount: 'Now, create an account.',
    onboardingCta: 'Get started',
    totalBalances: 'Total balance',
    totalReconciled: 'Reconciled',
    noBalances: 'No accounts yet.',
    lastBiggestIncome: 'Biggest income last month',
    lastBiggestExpense: 'Biggest expense last month',
    synthesisChart: 'Balance trend',
    accountsOverview: 'Accounts overview',
    noAccounts: 'No active accounts.',
    reportCharts: 'Reports',
  },
  settings: {
    profile: {
      title: 'Profile',
      email: 'Email address',
      submit: 'Save',
      stepUpHint: "You'll be asked to confirm with one of your passkeys before this is saved.",
      stepUpFailed: 'Passkey confirmation failed. Please try again.',
      success:
        "If this email isn't already registered to another account, check it for a link to confirm the change.",
      genericError: 'Something went wrong. Please try again.',
    },
    passkeys: {
      title: 'Passkeys',
      intro:
        'Manage the passkeys registered on your account. Passkeys are the only way to sign in.',
      stepUpHint:
        'Adding or removing a passkey first asks you to confirm with one you already have.',
      stepUpFailed: 'Passkey confirmation failed. Please try again.',
      device: 'Device',
      createdAt: 'Added',
      lastUsedAt: 'Last used',
      neverUsed: 'Never',
      unnamed: 'Unnamed passkey',
      empty: "You don't have any passkey yet.",
      deviceNameLabel: 'Name this passkey (optional)',
      deviceNamePlaceholder: 'e.g. MacBook Touch ID',
      add: 'Add a passkey',
      added: 'Passkey added',
      remove: 'Remove',
      removed: 'Passkey removed',
      lastPasskeyError:
        "This is your last passkey — you can't remove it, or you'd be permanently locked out.",
      genericError: 'Something went wrong. Please try again.',
    },
  },
  referenceData: {
    credit_card: 'Credit card',
    check: 'Check',
    cash_withdrawal: 'Cash withdrawal',
    transfer: 'Transfer',
    deposit: 'Deposit',
    direct_debit: 'Direct debit',
    initial_balance: 'Initial balance',
    salary: 'Salary',
    other_income: 'Other income',
    housing: 'Housing',
    rent: 'Rent',
    utilities: 'Utilities',
    food: 'Food',
    transport: 'Transport',
    leisure: 'Leisure',
    health: 'Health',
    other_expense: 'Other expense',
  },
  errors: {
    bank_not_active: 'This bank is not active.',
    bank_already_deleted: 'This bank is already deleted.',
    account_not_active: 'This account is not active.',
    account_already_deleted: 'This account is already deleted.',
    account_cannot_be_changed: 'The account cannot be changed.',
    bank_currency_immutable: 'The bank and currency cannot be changed.',
    bank_required: 'You must select a bank.',
    opening_operation_locked: 'The opening operation cannot be edited.',
    transfer_account_not_active: 'The transfer account is not active.',
    transfer_currency_mismatch: 'The transfer account must use the same currency.',
    transfer_account_invalid: 'Invalid transfer account.',
    transfer_same_account: 'You cannot transfer to the same account.',
    payment_method_invalid: 'This payment method does not match the operation type.',
    category_invalid: 'This category does not match the operation type.',
    step_up_required: 'Please confirm with your passkey first.',
    step_up_failed: 'Passkey confirmation failed. Please try again.',
    passkey_registration_failed: 'Passkey registration failed.',
    passkey_sign_in_failed: 'Passkey sign-in failed.',
    signup_link_invalid: 'This sign-up link is invalid or has expired.',
    email_change_link_invalid: 'This email change link has expired or was already used.',
    last_passkey:
      "This is your last passkey — you can't remove it, or you'd be permanently locked out.",
    quota_exceeded: 'You can have at most {limit} {kind}.',
    kinds: {
      banks: 'banks',
      accounts: 'accounts',
      schedulers: 'schedulers',
      reports: 'reports',
    },
  },
  language: {
    label: 'Language',
    // Each language's own name, in that language — never translated, so a
    // visitor can always find their language regardless of which one is
    // currently active.
    en: 'English',
    fr: 'Français',
    success: 'Language updated.',
    genericError: 'Something went wrong. Please try again.',
  },
};
